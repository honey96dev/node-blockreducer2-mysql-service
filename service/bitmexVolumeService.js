...
service.calculateVolume = () => {
  ...
  timestamp1 = timestamp1.toISOString();
  timestamp2 = timestamp2.toISOString();
  sql = sprintf("SELECT IFNULL(SUM(`size` * `price`), 0) / 100000000 `volume` FROM `%s_%s` WHERE `timestamp` > '%s' AND `timestamp` <= '%s';", dbTblName.tradesBuffer, symbol, timestamp1, timestamp2);
