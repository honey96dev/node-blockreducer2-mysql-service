...
service.renewSocket = () => {
    ...
    if (service.socketLastTimestamp > timestamp - service.timeoutDelay) {
        console.log('volumeService', 'renewSocket-still alive', service.socketLastTimestamp);
        return;
    }
    ...
    console.error('volumeService', 'renewSocket', service.socketLastTimestamp);
...
service.saveTradesBuffer = () => {
    ...
    console.log('saveTradesBuffer', service.tradeLastTimestamp);
};
...
service.calculateVolume = () => {
    ...
    sql = sprintf("DELETE FROM `%s` WHERE `timestamp` < '%s';", dbTblName.tradesBuffer, timestamp2);
    ...
    timestamp1 = timestamp1.toISOString();
    timestamp2 = timestamp2.toISOString();
    sql = sprintf("SELECT IFNULL(SUM(`price`), 0) `volume` FROM `%s` WHERE `timestamp` > '%s' AND `timestamp` <= '%s';", dbTblName.tradesBuffer, timestamp1, timestamp2);
    ...
    dbConn.query(sql, null, (error, rows, fields) => {
        ...
        sql = sprintf("INSERT INTO `%s`(`timestamp`, `volume`) VALUES('%s', '%s') ON DUPLICATE KEY UPDATE `volume` = VALUES(`volume`);", dbTblName.volume1m, volumeTimestamp1m, volume);
        dbConn.query(sql, null, (error, rows, fields) => {});
    });
    ...
    dbConn.query(sql, null, (error, rows, fields) => {
        ...
        const volume = rows[0]['volume'];
        sql = sprintf("INSERT INTO `%s`(`timestamp`, `volume`) VALUES('%s', '%s') ON DUPLICATE KEY UPDATE `volume` = VALUES(`volume`);", dbTblName.volume5m, volumeTimestamp5m, volume);
        dbConn.query(sql, null, (error, rows, fields) => {});
    });
    ...
    dbConn.query(sql, null, (error, rows, fields) => {
        ...
        const volume = rows[0]['volume'];
        sql = sprintf("INSERT INTO `%s`(`timestamp`, `volume`) VALUES('%s', '%s') ON DUPLICATE KEY UPDATE `volume` = VALUES(`volume`);", dbTblName.volume1h, volumeTimestamp1h, volume);
        dbConn.query(sql, null, (error, rows, fields) => {});
    });

    service.calcTimeoutId = setTimeout(service.calculateVolume, service.timeoutDelay);
    console.log('calculateVolume', service.tradeLastTimestamp);
};
