...
let service = {
    timeoutId: {},
    timeoutDelay: 100,
    checkRequestTimeoutId: null,
    checkRequestTimeoutDelay: 30000,
};

service.calculateFFT = (symbol, binSize, timestamp, endTimestamp) => {
    ...
    if (timestamp > endTimestamp) {
        return service.checkFixRequest();
    }
    let timeStep;
    if (binSize === '5m') {
        timeStep = 5 * 60 * 1000;
    ...
    dbConn.query(sql, null, (error, results, fields) => {
        ...
        service.timeoutId[timeoutIdKey] = setTimeout(service.calculateFFT, service.timeoutDelay, symbol, binSize, timestamp, endTimestamp);
    });
};

service.checkFixRequest = () => {
    if (service.checkRequestTimeoutId) {
        clearTimeout(service.checkRequestTimeoutId);
    }
    let sql = sprintf("SELECT * FROM `%s` WHERE `performed` = '%d' ORDER BY `id` LIMIT 1;", dbTblName.fftFixTask, 0);
    dbConn.query(sql, null, (error, rows, fields) => {
        if (error || rows.length === 0) {
            setTimeout(service.checkFixRequest, service.checkRequestTimeoutDelay)
        } else {
            const row = rows[0];
            service.calculateFFT(row['symbol'], row['binSize'], row['startTime'], row['endTime']);
            sql = sprintf("UPDATE `%s` SET `performed` = '%d' WHERE `id` = '%d';", dbTblName.fftFixTask, 1, row['id']);
            console.error('update', sql);
            dbConn.query(sql, null, (error, rows, fields) => {});
        }
    });
};
