    ...
    request(url, {timeout: 5000}, function (error, response, body) {
      ...
      if (response && response.statusCode === 200) {
        ...
        if (items.length > 0) {
          ...
          sql = sprintf("INSERT INTO `%s_%s_%s`(`timestamp`, `open`, `close`, `high`, `low`, `volume`) VALUES ? ON DUPLICATE KEY UPDATE `timestamp` = VALUES(`timestamp`), `open` = VALUES(`open`), `high` = VALUES(`high`), `low` = VALUES(`low`), `close` = VALUES(`close`), `volume` = VALUES(`volume`);", dbTblName.tradeBucketed, symbol, timeframe);
